...
def employeeFilter(request):
    #where select  from employee where name = "meet"
    employee = Employee.objects.filter(name ="meet").values()
    #selet  from employee where post = "Developer"
    employee2 = Employee.objects.filter(post ="developer").values()
    #select  from employee where name = "meet" and post = "developer"
    employee3 = Employee.objects.filter(name ="sahil",post ="c++ king").values()
    #select  from employee where name = "sahil" or post="c++ king"

    #>23
    #seelct  from employee where age > 23
    #employee4 = Employee.objects.filter(age>23).values()
    employee4 = Employee.objects.filter(age__gt=19).values()
    ...
    #contains
    employee8 = Employee.objects.filter(name__contains="m").values()
    employee9 = Employee.objects.filter(name__icontains="M").values()

    #startswith endswith
    employee10 = Employee.objects.filter(name__startswith="m").values()
    employee11 = Employee.objects.filter(name__endswith="R").values()
    employee12 = Employee.objects.filter(name__istartswith="M").values()
    employee13 = Employee.objects.filter(name__iendswith="m").values()

    #in
    employee14 = Employee.objects.filter(name__in=["meet","sahil"]).values()    

    #range
    employee15 = Employee.objects.filter(age__range=[15,25]).values()    
